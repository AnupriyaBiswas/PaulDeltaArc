...
const AnnualMaintenanceContracts = () => {
  ...
  const getCardStyle = (index) => {
    const position = (index - currentSlide + amcServices.length) % amcServices.length;
    if (position === 0) return 'scale-110 z-20 opacity-100';
    if (position === 1 || position === amcServices.length - 1) return 'scale-100 z-10 opacity-80';
    if (position === 2 || position === amcServices.length - 2) return 'scale-90 z-0 opacity-50';
    return 'hidden';
  };
  ...
  return (
    <div className='w-full bg-gradient-to-br from-teal-50 to-cyan-50 py-12 min-h-screen'>
      <div className='max-w-[1400px] mx-auto px-6'>
        ...
        {/* Services Section - Multi Card Centered Slider */}
        <div className='mb-12'>
            ...
            <div className='relative h-64 flex items-center justify-center'>
              ...
              <div className='relative flex items-center justify-center space-x-4'>
                {amcServices.map((service, index) => (
                  <div
                    key={index}
                    className={`transition-all duration-500 ease-in-out transform ${getCardStyle(index)} w-64`}
                  >
