...
const AnnualMaintenanceContracts = () => {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        // Turn on autoplay when component becomes visible
        if (entry.isIntersecting) {
          setIsAutoPlaying(true);
        } else {
          // Turn off autoplay when component is not visible
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      // Double-check visibility before advancing
  ...
  // Scroll active tab into view (only if component is visible)
  useEffect(() => {
    if (isVisible && serviceRefs.current[activeTab]) {
  ...
  return (
    <div ref={componentRef} className='w-full bg-gradient-to-br from-teal-50 to-cyan-50 py-6 text-sm'>
