...
const AnnualMaintenanceContracts = () => {
  ...
  const extendedServices = [
    ...amcServices.slice(-1), // Last item at the beginning
    ...amcServices,
    ...amcServices.slice(0, 1) // First item at the end
  ];

  const [displayIndex, setDisplayIndex] = useState(1); // Start at index 1 (actual first item)
  ...
  useEffect(() => {
    if (displayIndex === 0) {
      // At the duplicate last slide, jump to actual last slide
      setTimeout(() => {
        setIsTransitioning(false);
        setDisplayIndex(amcServices.length);
        setTimeout(() => setIsTransitioning(true), 50);
      }, 500);
    } else if (displayIndex === extendedServices.length - 1) {
      // At the duplicate first slide, jump to actual first slide
      setTimeout(() => {
        setIsTransitioning(false);
        setDisplayIndex(1);
        setTimeout(() => setIsTransitioning(true), 50);
  ...
  }, [displayIndex, amcServices.length, extendedServices.length]);
  ...
  const goToSlide = (index) => {
    setDisplayIndex(index + 1); // +1 because of the duplicate at the beginning
  };

  // Get current slide for indicators
  const getCurrentSlide = () => {
    if (displayIndex === 0) return amcServices.length - 1;
    if (displayIndex === extendedServices.length - 1) return 0;
