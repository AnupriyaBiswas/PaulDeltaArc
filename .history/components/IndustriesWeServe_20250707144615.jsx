    ...
    <div className="w-full bg-gradient-to-br from-teal-50 to-cyan-50 py-6 px-2 md:px-10 min-h-screen flex flex-col justify-center items-center">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4 w-full max-w-7xl">
        {industries.map((industry, index) => (
          <div
            key={index}
            className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 overflow-hidden cursor-pointer flex flex-col h-[280px]"
          >
            {/* Image */}
            <div className="absolute h-40 w-full flex-shrink-0">
              <Image
                src={industry.image}
                alt={industry.title}
                fill
            ...
            {/* Content */}
            <div className={`flex flex-col justify-start p-4 bg-gradient-to-br ${industry.color} text-white flex-1`}>
