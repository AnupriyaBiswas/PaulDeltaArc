...
const AnnualMaintenanceContracts = () => {
  ...
  useEffect(() => {
    if (serviceRefs.current[activeTab]) {
      serviceRefs.current[activeTab].scrollIntoView({ behavior: 'smooth', block: 'nearest', inline: 'center' });
    }
  }, [activeTab]);

  return (
    <div className='w-full bg-gradient-to-br from-teal-50 to-cyan-50 py-10 text-[14px]'>
      <div className='max-w-7xl mx-auto px-4'>
        <div className='text-center mb-6'>
          <h1 className='text-3xl md:text-4xl font-bold text-[#2c3e50] mb-3'>📄 Annual Maintenance Contracts</h1>
        ...
        <div className='mb-10 flex flex-col lg:flex-row gap-6'>
          {/* Left - Scrollable services */}
          <div className='lg:w-1/2 max-h-[360px] overflow-y-auto pr-3'>
            <div className='sticky top-0 bg-teal-50 z-10 pb-3'>
          ...
          <div className='lg:w-1/2'>
            <div className='bg-white rounded-xl shadow-md p-6 min-h-[360px]'>
              <div className='flex items-center space-x-4 mb-4'>
                ...
                <div>
                  ...
                  <p className='text-[13px] text-gray-600'>{amcServices[activeTab].description}</p>
                </div>
              </div>

              <div className='border-t pt-4'>
                <p className='text-[13px] text-gray-700 mb-4 leading-relaxed'>{amcServices[activeTab].details}</p>
                <h4 className='text-base font-semibold text-[#2c3e50] mb-2'>Key Features:</h4>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-5'>
          {benefits.map((benefit, index) => (
            <div
              key={index}
              className='group bg-white p-5 rounded-xl shadow border border-gray-200 hover:shadow-md transition-all duration-300 hover:-translate-y-1'
            >
              ...
              <h4 className='text-[15px] font-semibold text-[#2c3e50] mb-1 text-center'>{benefit.title}</h4>
              <p className='text-[13px] text-gray-600 text-center leading-relaxed'>{benefit.description}</p>
            </div>
          ))}
        </div>
      </div>
