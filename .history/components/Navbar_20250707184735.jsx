...
const Navbar = () => {
  ...
  return (
    <div
      ...
      className={
        shadow
          ? 'fixed w-full h-16 shadow-2xl z-[100] ease-in-out duration-300 backdrop-blur-md border-b border-gray-200'
          : 'fixed w-full h-16 z-[100] backdrop-blur-md border-b border-gray-100'
      }
    >
      <div className='flex justify-between items-center w-full h-full px-4 2xl:px-16'>
        ...
        <div>
          <ul style={{ color: `${linkColor}` }} className='hidden md:flex items-center'>
            <li className='ml-6 text-sm font-semibold uppercase hover:text-teal-600 transition-all duration-300'>
              <Link href='/'>
                <a className='py-2 px-2 relative group'>
                  Home
                  <span className='absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-teal-500 to-cyan-500 transition-all duration-300 group-hover:w-full rounded-full'></span>
                </a>
              </Link>
            </li>
            <li className='ml-6 text-sm font-semibold uppercase hover:text-teal-600 transition-all duration-300'>
            ...
            >
              <div className='flex items-center gap-1 py-2 px-2 hover:text-teal-600 transition-all duration-300 relative group'>
                Services 
              ...
              {/* Dropdown Menu */}
              <div 
                className={`absolute top-full left-0 mt-1 w-80 bg-white/95 backdrop-blur-md shadow-2xl rounded-2xl overflow-hidden text-gray-800 z-50 border border-white/20 transition-all duration-300 ${
                ...
                <div className='py-3'>
                  {services.map((item, idx) => (
                    <Link href={item.link} key={idx}>
                      <a className='block px-6 py-3 hover:bg-gradient-to-r hover:from-teal-50/80 hover:to-cyan-50/80 hover:text-teal-700 transition-all duration-300 text-sm font-medium border-b border-gray-100/50 last:border-b-0 hover:shadow-sm'>
                        {item.name}
                      </a>
                    </Link>
                  ))}
          ...
          <div
            ...
            onClick={handleNav}
            className='md:hidden p-2 rounded-xl hover:bg-gradient-to-br hover:from-teal-50 hover:to-cyan-50 transition-all duration-300 hover:shadow-lg'
          >
